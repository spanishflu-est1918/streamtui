...
fn test_status_parsing() {
    ...
    assert!((status.volume - 0.8).abs() < 0.01, "Volume should be 0.8 (80%)");
}
...
async fn test_cast_failure_recovery() {
    ...
    assert!(manager.selected().is_some(), "Device should still be selected after failure");
...
async fn test_discovery_during_cast() {
    ...
    assert!(discover_result.is_ok(), "Discovery should not timeout/deadlock");
}
...
fn test_playback_progress_zero_duration() {
    ...
    let progress = status.progress();
    assert!((progress - 0.0).abs() < f32::EPSILON, "Progress should be 0 with zero duration");
}
...
fn test_format_position() {
    ...
    assert!(pos_str.contains("01") || pos_str.contains("1:01"), "Should format as HH:MM:SS");
    
    let dur_str = status.format_duration();
    assert!(dur_str.contains("2:00") || dur_str.contains("02:00"), "Should format duration");
}
